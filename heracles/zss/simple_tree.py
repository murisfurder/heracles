...
class SetNode(Node):
    ...
    def get_permutations(self):
        return [ x for x in permutations(self.children) ]

class NodeTreesGenerator(object):
    def __init__(self, root):
        self.root = root
        self.set_nodes = tuple(root.iter_set_nodes())

    def iter_permutations(self):
        permutations = [ x.get_permutations() for x in self.set_nodes ]
        for p in product(*permutations):
            yield p

    def get_permutated_node(self, node, perm):
        n_node = Node(node.label)
        if isinstance(node, SetNode):
            i = self.set_nodes.index(node)
            children = perm[i]
        else:
            children = node.children
        n_node.children = [ self.get_permutated_node(x, perm) for x in children ]
        return n_node

    def __iter__(self):
        for permutation in self.iter_permutations():
            yield self.get_permutated_node(self.root, permutation)
